class Solution {
public:

    // int dfs(vector<vector<int>>&grid , int &length , 
    //         int start, int i ,int j,vector<vector<int>>&vis){
    //     if(vis[i][j] == 1) return length;
    //     if(start == 1){
    //         dfs(grid ,length + 1 ,2,dx,dy,vis);
    //     }
    //     if(start == 2){
    //         dfs(grid ,length + 1 ,0,dx,dy,vis);
    //     }
    //     if(start == 0){
    //         dfs(grid ,length + 1 ,2,dx,dy,vis);
    //     }


    //     return length;
    // }



    //youtube soln
    vector<vector<int>>directions = {{1,1},{1,-1},{-1,-1},{-1,1}};
    
    int m ,n;
    int result;
    int solve(int i , int j , int d , bool canTurn , int val ,
               vector<vector<int>>&grid ){
            
            int i_ = i + directions[d][0];
            int j_ = j + directions[d][1];

            if(i_ < 0 || i_ >= n || j_ < 0 || j_ >= m || grid[i_][j_] != val) {
                return 0;
            }

            int result =0;
            int keepMoving = 1+solve(i_,j_,d,canTurn,val ==2 ? 0 : 2, grid);
            result = max(result,keepMoving);

            if(canTurn){
                int turnAndMove = max(keepMoving , 1+solve(i_,j_, (d+1)%4 ,false,val ==2?0:2 , grid));
                result = max(result,turnAndMove);
            }
            return result;
        } 


    int lenOfVDiagonal(vector<vector<int>>& grid) {
        // iterate array {{-1,-1},{-1,1},{+1,+1},{+1,-1}};
        // make agtmost one turn 
        // series 2,0,2,0,2,0,2,0

        // dfs

         n = grid.size();
         m = grid[0].size();

        for(int i = 0; i<n;i++){
            for(int j = 0; j<m;j++){
                if(grid[i][j] == 1 ){
                   for(int k=0;k<=3;k++){
                    result = max(result, 1+solve(i,j,k,true,2,grid));
                   }
                }
            }
        }

        return result;
    }
};